...
class AutosummaryRenderer:
    ...
    def __init__(self, app: Sphinx) -> None:
        ...
        # use the templates folder relative to this file
...
def generate_autosummary_content(
    ...
    if doc.objtype == 'module':
        ...
        ns['functions_imported'], _ = _get_members(
            doc, app, obj, {'function'}, imported='only', as_dict=True
        )
        ns['classes_imported'], _ = _get_members(
            doc, app, obj, {'class'}, imported='only', as_dict=True
        )
        ns['exceptions_imported'], _ = _get_members(
            doc, app, obj, {'exception'}, imported='only', as_dict=True
        )
        ns['functions_local'], _ = _get_members(
            doc, app, obj, {'function'}, imported='never'
        )
        ns['classes_local'], _ = _get_members(
            doc, app, obj, {'class'}, imported='never'
        )
        ns['exceptions_local'], _ = _get_members(
            doc, app, obj, {'exception'}, imported='never'
        )

        # can't distinguish imported attributes
    ...
    *,
    ...
    for name, value in all_members.items():
        ...
        if documenter.objtype in types:
            ...
            # logic about whether the item is imported and whether to accept it
            if imported == 'only':
                do_include = not (getattr(value, '__module__', None) == obj.__name__)
            elif imported == 'never':
                do_include = (getattr(value, '__module__', None) == obj.__name__)
            ...
            if do_include:
                ...
                def public_include(name):
                    thisobj = getattr(obj, name)
                    if not as_dict:
                        public.append(name)
                        return
                    modname = getattr(value, '__module__', None)
                    if modname is not None:
                        tryname = f"{modname}.{name}"
                        try:
                            mod = import_module(modname)
                        except ImportError:
                            # don't insert if the module can't be found or loaded
                            return
                        modobj = getattr(mod, name, None)
                    if modobj is thisobj:
                        # confirmed we've found the original and tryname is correct
                        pass
                    else:
                        for mn in dir(mod):
                            modobj = getattr(mod, mn)
                            if modobj is thisobj:
                                tryname = f"{modname}.{mn}"
                                break
                        else:
                            # couldn't find the object in the module, so return
                            return
                    public[name] = tryname
